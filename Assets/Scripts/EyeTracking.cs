...
public class EyeTracking : MonoBehaviour
{
    ...
    void LogGazeData(VarjoEyeTracking.GazeData data, VarjoEyeTracking.EyeMeasurements eyeMeasurements)
    {
        ...
        logData[7] = invalid ? "" : (Quaternion.Euler(0, -90, 0) * xrCamera.transform.localRotation * data.gaze.forward).ToString("F3");
    ...
    public void StartLogging(Environment environment)
    {
        if (logging)
        {
        ...
        // string fileName = string.Format("{0}-{1:00}-{2:00}-{3:00}-{4:00}-{5:00}", now.Year, now.Month, now.Day, now.Hour, now.Minute, now.Second);
        // string fileName = $"UserId-{userId}-session-{session}-ContId-{environment.m_ContentIndex}-WRot-{environment.m_WorldRotation}-ConSca-{environment.m_ContextScalingFactor}-FocSca-{environment.m_FocusScalingFactor}";
        string fileName = $"{userId}-{session}-{environment.m_ContentIndex}-{environment.m_WorldRotation}-{environment.m_ContextScalingFactor}-" + string.Format("{0}-{1:00}-{2:00}-{3:00}-{4:00}-{5:00}", now.Year, now.Month, now.Day, now.Hour, now.Minute, now.Second);
